...
def generate_demographic_data_final(num_records):
    ...
    for patient_id in range(1, num_records + 1):
        ...
        functional_independence = fake.random_element(
            elements=('Independent', 'Assistance Required', 'Dependent'))  # Functional independence level
        ...
        record = {
            ...
            'Active_Stay_Ind': active_stay_ind,  # Active stay indicator as 0 or 1
...
def generate_diagnosis_data_based_on_demographic(demographic_data):
    ...
    for patient in demographic_data:
        ...
        discharge_date = datetime.now().date() if patient["Active_Stay_Ind"] == 1 else datetime.strptime(patient["discharge_date"], '%Y-%m-%d').date()
